...
const NPM_LATEST_URL = `https://registry.npmjs.org/${CLI_PACKAGE_NAME}/latest`;
...
async function readCurrentVersion(): Promise<string> {
  const packageJson = JSON.parse(await readPackagedText("package.json")) as PackageJson;
...
async function fetchLatestVersion(): Promise<string> {
  ...
  if (!pkg.version) {
    throw new Error("npm registry response does not include version");
  }
  return pkg.version;
}
...
async function updateNoticeForSkill(): Promise<string> {
  try {
    ...
    return [
      `> Update available: ${CLI_PACKAGE_NAME} ${status.currentVersion} -> ${status.latestVersion}`,
      "> Ask the user before updating the global CLI.",
      `> ${updateCommandHint().replace(/\n/g, "\n> ")}`,
      "",
    ].join("\n");
  } catch (error) {
    return [
      `> Update check failed: ${error instanceof Error ? error.message : String(error)}`,
      `> Run \`${CLI_BIN_NAME} version\` to retry when registry access is available.`,
      "",
    ].join("\n");
  }
}
...
async function commandVersion(): Promise<void> {
  const status = await checkForUpdate();
  console.log(`${CLI_PACKAGE_NAME} ${status.currentVersion}`);
  console.log(`latest ${status.latestVersion}`);
  if (status.isOutdated) {
    console.log(`update available ${status.currentVersion} -> ${status.latestVersion}`);
    console.log(updateCommandHint());
    return;
  }
...
async function commandSkill(): Promise<void> {
  process.stdout.write(await updateNoticeForSkill());
